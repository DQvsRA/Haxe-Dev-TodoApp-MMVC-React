...
app_controller_signals_InfoPopupMediatorNotificationSignal.__super__ = msignal_Signal2;
app_controller_signals_InfoPopupMediatorNotificationSignal.prototype = $extend(msignal_Signal2.prototype,{
	__class__: app_controller_signals_InfoPopupMediatorNotificationSignal
});
...
app_model_TodoModel.prototype = $extend(mmvc_impl_Actor.prototype,{
	...
	,updateTodo: function(index,text,callback) {
		haxe_Log.trace("-> updateTodo: id = " + index + " text = " + text,{ fileName : "TodoModel.hx", lineNumber : 47, className : "app.model.TodoModel", methodName : "updateTodo"});
		var todoVO = this.getTodoByIndex(index);
		var tmp = this._serverService;
		var tmp1 = this.SERVER_TODO_ROUTE + "/" + todoVO.id;
		var _g = new haxe_ds_StringMap();
...
app_view_mediators_TodoListMediator.prototype = $extend(mmvc_impl_Mediator.prototype,{
	...
	,handleTodoListAction: function(index,action,callback,data) {
		...
		case 0:
			if(callback != null) {
				this.toggleTodoSignal.complete.addOnce(callback);
			}
			this.toggleTodoSignal.dispatch(index);
			break;
		case 1:
			if(callback != null) {
				this.updateTodoSignal.complete.addOnce(callback);
			}
			var text = Std.string(data);
			this.updateTodoSignal.dispatch(index,text);
			break;
		case 2:
			if(callback != null) {
				this.deleteTodoSignal.complete.addOnce(callback);
			}
			this.deleteTodoSignal.dispatch(index);
			break;
		}
	}
	,handleNotification: function(type,data) {
...
app_controller_commands_prepare_PrepareCompleteCommand.__meta__ = { fields : { todoModel : { type : ["app.model.TodoModel"], inject : null}, infoPopupNotificationSignal : { type : ["app.controller.signals.InfoPopupMediatorNotificationSignal"], inject : null}, todoListNotificationSignal : { type : ["app.controller.signals.TodoListMediatorNotificationSignal"], inject : null}}};
...
app_controller_commands_todo_CreateTodoCommand.__meta__ = { fields : { infoPopupMediatorSignal : { type : ["app.controller.signals.InfoPopupMediatorNotificationSignal"], inject : null}, todoListMediatorSignal : { type : ["app.controller.signals.TodoListMediatorNotificationSignal"], inject : null}, todoFormMediatorSignal : { type : ["app.controller.signals.TodoFormMediatorNotificationSignal"], inject : null}, todoModel : { type : ["app.model.TodoModel"], inject : null}, text : { type : ["String"], inject : null}}};
...
app_controller_signals_TodoListMediatorNotificationSignal.PREFIX = "todolist_mediator_signal__";
